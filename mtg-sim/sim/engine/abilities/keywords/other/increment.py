...
def _creature_stats(perm: Permanent, game: GameState | None = None) -> tuple[int, int]:
    return effective_power(perm, game), effective_toughness(perm, game)
...
def increment_triggers(
    ...
    if not is_creature(perm):
        return False
    power, toughness = _creature_stats(perm, game)
    ...
    *,
    ...
    for perm in game.zones.battlefield:
        if perm.controller_idx != controller_idx or not has_increment(perm):
            continue
        if not increment_triggers(perm, mana_spent, game):
            continue
